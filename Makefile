...
SHARED_LIBS := $(CORE_PACK_LIB) $(CORE_PANE_LIB) $(CORE_LAYOUT_LIB) $(CORE_PANE_MODULE_LIB) $(CORE_BASE_LIB)
...
.PHONY: all build clean run run-headless test visual-harness identity print-identity \
	export-snapshot-json snapshot-bridge-check \
