...
/*
 * Canonical indexed tileset packs intentionally store only texture-project
 * data.  They are source assets, not saved editor workspaces, so they do not
 * carry DPS3/DPS2 shell state.  Apply the validated active surface to the
 * already-seeded application shell instead of treating that omission as a
 * malformed drawing project.
 */
static CoreResult
drawing_program_snapshot_apply_texture_only_indexed_project(DrawingProgramAppContext *ctx,
                                                            DrawingProgramTextureProject *loaded_project) {
    CoreResult result;
    if (!ctx || !loaded_project ||
        loaded_project->profile_kind != DRAWING_PROGRAM_TEXTURE_PROJECT_PROFILE_INDEXED_ATLAS_V1 ||
        loaded_project->surface_count == 0u || loaded_project->active_surface_index >= loaded_project->surface_count) {
        return drawing_program_snapshot_invalid("invalid texture-only indexed project");
    }
    drawing_program_texture_project_dispose(&ctx->texture_project);
    ctx->texture_project = *loaded_project;
    memset(loaded_project, 0, sizeof(*loaded_project));
    result = drawing_program_texture_project_session_activate_loaded_surface(ctx);
    if (result.code == CORE_OK) {
        if (!drawing_program_indexed_editor_tool_allowed(ctx->editor.active_tool)) {
...
CoreResult drawing_program_snapshot_save(struct DrawingProgramAppContext *ctx, const char *path) {
    ...
    result = drawing_program_authoring_host_export_accepted_pane_state(
        ctx, &legacy_payload->layout_state, legacy_payload->nodes, &legacy_payload->header.node_count,
        &accepted_root_index, legacy_payload->bindings, &legacy_payload->header.binding_count);
    if (result.code != CORE_OK) {
        free(legacy_payload);
        return result;
    }
    ...
    if (snapshot_trace_enabled()) {
        fprintf(stderr,
                "drawing_program trace snapshot_save begin path=%s tool=%u "
                "theme=%u font=%u zoom=%d slot_l=%u slot_r=%u color=%u\n",
                path ? path : "(null)", (unsigned)ctx->editor.active_tool, (unsigned)ctx->ui.theme_preset_id,
                (unsigned)ctx->ui.font_preset_id, (int)ctx->ui.font_zoom_step, (unsigned)ctx->ui.left_panel_slot,
                (unsigned)ctx->ui.right_panel_slot, (unsigned)ctx->ui.active_color_index);
    }
    ...
    if (result.code != CORE_OK) {
        ...
        if (snapshot_trace_enabled()) {
            fprintf(stderr,
                    "drawing_program trace snapshot_save fail path=%s code=%d "
                    "message=%s\n",
                    path ? path : "(null)", (int)result.code, result.message ? result.message : "(null)");
        }
        return result;
    }
    ...
    free(legacy_payload);
    if (snapshot_trace_enabled()) {
        fprintf(stderr, "drawing_program trace snapshot_save end path=%s code=%d message=%s\n", path ? path : "(null)",
                (int)result.code, result.message ? result.message : "(null)");
    }
    return result;
}
...
CoreResult drawing_program_snapshot_load(struct DrawingProgramAppContext *ctx, const char *path) {
    ...
    uint8_t loaded_texture_only_indexed_project = 0u;
    ...
    if (result.code != CORE_OK) {
        ...
        if (snapshot_trace_enabled()) {
            fprintf(stderr,
                    "drawing_program trace snapshot_load fail_open path=%s code=%d "
                    "message=%s\n",
                    path ? path : "(null)", (int)result.code, result.message ? result.message : "(null)");
        }
        return result;
    }
    texture_project_result =
        drawing_program_texture_project_snapshot_load(&loaded_texture_project, &reader, &loaded_texture_project_found);
    ...
    if (snapshot_trace_enabled()) {
        fprintf(stderr,
                "drawing_program trace snapshot_load shell_result code=%d "
                "message=%s found=%u\n",
                (int)shell_result.code, shell_result.message ? shell_result.message : "(null)",
    ...
    if (!loaded_current_shell) {
        ...
        if (result.code != CORE_OK) {
            if (result.code == CORE_ERR_NOT_FOUND && loaded_texture_project_found &&
                loaded_texture_project.profile_kind == DRAWING_PROGRAM_TEXTURE_PROJECT_PROFILE_INDEXED_ATLAS_V1) {
                loaded_texture_only_indexed_project = 1u;
            } else {
                if (current_shell_format_incompatible) {
                    ...
                    if (snapshot_trace_enabled()) {
                        fprintf(stderr,
                                "drawing_program trace snapshot_load fail path=%s code=%d "
                                "message=%s\n",
                                path ? path : "(null)", (int)shell_result.code,
        ...
        if (!loaded_texture_only_indexed_project && chunk.size != (uint64_t)sizeof(*payload)) {
        ...
        } else if (!loaded_texture_only_indexed_project) {
    ...
    if (!loaded_texture_only_indexed_project) {
        ...
        if (snapshot_trace_enabled()) {
            fprintf(stderr,
                    "drawing_program trace snapshot_load pre_raster_init "
                    "logical=%ux%u density=%u layer_count=%u raster=%ux%u samples=%u "
                    "next_layer=%u schema=%u\n",
                    (unsigned)ctx->document.logical_width, (unsigned)ctx->document.logical_height,
                    (unsigned)ctx->document.sample_density, (unsigned)ctx->document.layer_count,
                    (unsigned)ctx->document.raster_width, (unsigned)ctx->document.raster_height,
                    (unsigned)ctx->document.raster_sample_count, (unsigned)ctx->document.next_layer_id,
        ...
        if (result.code != CORE_OK) {
            ...
            if (snapshot_trace_enabled()) {
                fprintf(stderr,
                        "drawing_program trace snapshot_load layer_raster_init_fail "
                        "code=%d message=%s logical=%ux%u density=%u layer_count=%u "
                        "raster=%ux%u samples=%u\n",
                        (int)result.code, result.message ? result.message : "(null)",
                        (unsigned)ctx->document.logical_width, (unsigned)ctx->document.logical_height,
                        (unsigned)ctx->document.sample_density, (unsigned)ctx->document.layer_count,
                        (unsigned)ctx->document.raster_width, (unsigned)ctx->document.raster_height,
    ...
    if (snapshot_trace_enabled() && result.code != CORE_OK) {
        fprintf(stderr, "drawing_program trace snapshot_load fail path=%s code=%d message=%s\n", path ? path : "(null)",
                (int)result.code, result.message ? result.message : "(null)");
    }
    if (snapshot_trace_enabled()) {
        fprintf(stderr,
                "drawing_program trace snapshot_load end path=%s code=%d tool=%u "
                "theme=%u font=%u zoom=%d slot_l=%u slot_r=%u color=%u leafs=%u "
                "upgraded_palette=%u\n",
                path ? path : "(null)", (int)result.code, (unsigned)ctx->editor.active_tool,
                (unsigned)ctx->ui.theme_preset_id, (unsigned)ctx->ui.font_preset_id, (int)ctx->ui.font_zoom_step,
                (unsigned)ctx->ui.left_panel_slot, (unsigned)ctx->ui.right_panel_slot,
                (unsigned)ctx->ui.active_color_index, (unsigned)ctx->pane_host.leaf_count,
