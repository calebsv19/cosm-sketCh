...
enum {
    DRAWING_PROGRAM_TEXTURE_EXPORT_MANIFEST_SCHEMA_VERSION = 5u
};
...
static const char *texture_export_primitive_kind_name(uint32_t primitive_kind) {
    switch ((DrawingProgramTexturePrimitiveKind)primitive_kind) {
        case DRAWING_PROGRAM_TEXTURE_PRIMITIVE_KIND_PLANE: return "PLANE";
        case DRAWING_PROGRAM_TEXTURE_PRIMITIVE_KIND_RECT_PRISM: return "RECT_PRISM";
        case DRAWING_PROGRAM_TEXTURE_PRIMITIVE_KIND_NONE:
        default: return "NONE";
    }
}

static const char *texture_export_binding_kind_name(uint32_t binding_kind) {
    switch ((DrawingProgramTextureBindingKind)binding_kind) {
        case DRAWING_PROGRAM_TEXTURE_BINDING_KIND_SEPARATE_FACES: return "SEPARATE_FACES";
        case DRAWING_PROGRAM_TEXTURE_BINDING_KIND_ATLAS: return "ATLAS";
        case DRAWING_PROGRAM_TEXTURE_BINDING_KIND_NONE:
        default: return "NONE";
    }
}
    ...
    *out_height = 0u;
    ...
    for (sample_index = 0u; sample_index < document->raster_sample_count; ++sample_index) {
        ...
        for (layer_index = 0u; layer_index < document->layer_count; ++layer_index) {
            ...
            if (!layer->visible ||
                !texture_export_layer_included_for_lane(role_kind,
                                                       export_intent_kind,
                                                       lane_kind)) {
                continue;
            }
...
static json_object *texture_export_build_surface_json(const DrawingProgramTextureSurface *surface,
    ...
    if (layer_material_intent_kind_by_index && layer_material_intent_count > 0u) {
...
CoreResult drawing_program_texture_export_current_project(DrawingProgramAppContext *ctx,
    ...
    if (!manifest || !surfaces || !base_surfaces || !overlay_surfaces) {
        if (manifest) {
    ...
    json_object_object_add(manifest,
                           "schema_version",
                           json_object_new_int((int)DRAWING_PROGRAM_TEXTURE_EXPORT_MANIFEST_SCHEMA_VERSION));
    json_object_object_add(manifest,
                           "export_binding_kind",
                           json_object_new_string(texture_export_binding_kind_name(ctx->texture_project.export_binding_kind)));
    json_object_object_add(manifest,
                           "export_intent_kind",
                           json_object_new_string(
    ...
    json_object_object_add(manifest,
                           "primitive_kind",
                           json_object_new_string(texture_export_primitive_kind_name(ctx->texture_project.primitive_kind)));
    json_object_object_add(manifest,
                           "quality_preset",
                           json_object_new_string(
    ...
    json_object_object_add(manifest, "surface_count", json_object_new_int((int)ctx->texture_project.surface_count));
    json_object_object_add(manifest, "surfaces", surfaces);
    ...
    if (emitted_output_kind == DRAWING_PROGRAM_TEXTURE_EXPORT_INTENT_KIND_BASE_PLUS_OVERLAY) {
        json_object_object_add(manifest,
    ...
    for (i = 0u; i < ctx->texture_project.surface_count; ++i) {
        ...
        if (surface->face_role == DRAWING_PROGRAM_TEXTURE_FACE_ROLE_UNSPECIFIED) {
            (void)snprintf(png_name, sizeof(png_name), "%s_surface_%02u%s.png", project_stem, (unsigned)(i + 1u),
                           texture_export_lane_file_suffix(emitted_output_kind == DRAWING_PROGRAM_TEXTURE_EXPORT_INTENT_KIND_BASE_PLUS_OVERLAY
        ...
        result = texture_export_compose_surface_rgba(document,
                                                     ...
                                                     layer_role_count,
                                                     ctx->texture_project.export_intent_kind,
                                                     emitted_output_kind == DRAWING_PROGRAM_TEXTURE_EXPORT_INTENT_KIND_BASE_PLUS_OVERLAY
                                                         ? DRAWING_PROGRAM_TEXTURE_EXPORT_LANE_KIND_BASE
                                                         : DRAWING_PROGRAM_TEXTURE_EXPORT_LANE_KIND_FLATTENED,
                                                     &rgba,
                                                     &width,
        ...
        surface_json = texture_export_build_surface_json(surface,
                                                         document,
                                                         png_name,
                                                         layer_material_intent_kind_by_index,
                                                         layer_material_intent_count);
        if (!surface_json) {
            json_object_put(manifest);
            return (CoreResult){ CORE_ERR_OUT_OF_MEMORY, "failed to allocate texture export surface manifest" };
        }
        json_object_array_add(surfaces, surface_json);
