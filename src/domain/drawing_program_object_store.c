...
int drawing_program_object_store_selection_contains_object_id(
    const DrawingProgramObjectSelectionState *selection, uint32_t object_id) {
    uint32_t i;
...
static CoreResult object_store_prepare_record(const DrawingProgramObjectRecord *seed,
    ...
    if (!seed || !out_record || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object record seed");
    }
    if (seed->layer_id == 0u) {
        return drawing_program_object_store_invalid("object layer_id must be non-zero");
    }
    if (!object_type_valid(seed->type)) {
        return drawing_program_object_store_invalid("unsupported object type");
    }
    if (seed->width == 0u || seed->height == 0u) {
        return drawing_program_object_store_invalid("object bounds must be non-zero");
    }
    ...
    *out_record = *seed;
    ...
    if (out_record->type != (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_PATH) {
        ...
        if (!drawing_program_object_path_payload_valid(out_record)) {
            return drawing_program_object_store_invalid("invalid path payload");
        }
    }
...
CoreResult drawing_program_object_store_find_index_for_id(const DrawingProgramObjectStore *store,
    ...
    if (!store || !out_index || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object id lookup");
    }
...
CoreResult drawing_program_object_store_insert_with_id(DrawingProgramObjectStore *store,
    ...
    if (!store || !seed || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object insert request");
    }
...
CoreResult drawing_program_object_store_add(DrawingProgramObjectStore *store,
    ...
    CoreResult result;
    if (!store || !seed) {
        return drawing_program_object_store_invalid("invalid object add request");
    }
    if (store->next_object_id == 0u) {
        store->next_object_id = 1u;
    }
...
CoreResult drawing_program_object_store_add_path(DrawingProgramObjectStore *store,
    ...
    if (!store || !payload || !seed_style) {
        return drawing_program_object_store_invalid("invalid path add request");
    }
    ...
    if (payload->point_count > DRAWING_PROGRAM_OBJECT_PATH_MAX_POINTS) {
        return drawing_program_object_store_invalid("path point count exceeds capacity");
    }
...
CoreResult drawing_program_object_store_remove_by_id(DrawingProgramObjectStore *store,
    ...
    CoreResult result;
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object remove request");
    }
...
CoreResult drawing_program_object_store_replace_all(DrawingProgramObjectStore *store,
    ...
    CoreResult result;
    if (!store) {
        return drawing_program_object_store_invalid("invalid object replace request");
    }
    if (record_count > 0u && !records) {
        return drawing_program_object_store_invalid("missing replacement object records");
    }
    if (record_count > DRAWING_PROGRAM_MAX_OBJECTS) {
        return drawing_program_object_store_invalid("replacement object count exceeds capacity");
    }
...
CoreResult drawing_program_object_store_set_origin(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object set-origin request");
    }
    result = drawing_program_object_store_find_index_for_id(store, object_id, &index);
    if (result.code != CORE_OK) {
        return result;
    ...
    if (store->objects[index].type == (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_PATH &&
        ...
        for (i = 0u; i < (uint32_t)store->objects[index].path_point_count; ++i) {
            ...
            if (next_x < (int64_t)INT32_MIN || next_x > (int64_t)INT32_MAX ||
                ...
                return drawing_program_object_store_invalid("path move origin delta out of range");
            }
...
CoreResult drawing_program_object_store_set_size(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object size set request");
    }
    if (width == 0u) {
        width = 1u;
    }
    ...
    if (store->objects[index].type != (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_RECT &&
        ...
        return drawing_program_object_store_invalid("object size set requires RECT or ELLIPSE object type");
    }
...
CoreResult drawing_program_object_store_set_stroke_width(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object stroke-width set request");
    }
    if (stroke_width < 1u) {
        stroke_width = 1u;
    }
...
CoreResult drawing_program_object_store_set_style_mode(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object style-mode set request");
    }
...
CoreResult drawing_program_object_store_set_path_closed(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-closed set request");
    }
    result = drawing_program_object_store_find_index_for_id(store, object_id, &index);
    if (result.code != CORE_OK) {
        return result;
    ...
    if (store->objects[index].type != (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_PATH) {
        return drawing_program_object_store_invalid("path closed set requires PATH object type");
    }
    ...
    if (closed && store->objects[index].path_point_count < 3u) {
        return drawing_program_object_store_invalid("closing path requires at least 3 points");
    }
...
CoreResult drawing_program_object_store_set_path_payload(DrawingProgramObjectStore *store,
    ...
    if (!store || !payload || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-payload set request");
    }
    result = drawing_program_object_store_find_index_for_id(store, object_id, &index);
    if (result.code != CORE_OK) {
        return result;
    ...
    if (store->objects[index].type != (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_PATH) {
        return drawing_program_object_store_invalid("path payload set requires PATH object type");
    }
...
CoreResult drawing_program_object_store_set_path_point(DrawingProgramObjectStore *store,
    ...
    CoreResult result;
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-point set request");
    }
    ...
    if ((uint32_t)point_index >= payload.point_count) {
        return drawing_program_object_store_invalid("path-point index out of range");
    }
    if (out_previous_x) {
...
CoreResult drawing_program_object_store_set_path_point_data(DrawingProgramObjectStore *store,
    ...
    if (!store || object_id == 0u || !point) {
        return drawing_program_object_store_invalid("invalid object path-point data set request");
    }
    result = drawing_program_object_store_get_path_payload(store, object_id, &payload);
    if (result.code != CORE_OK) {
        return result;
    ...
    if ((uint32_t)point_index >= payload.point_count) {
        return drawing_program_object_store_invalid("path-point data index out of range");
    }
...
CoreResult drawing_program_object_store_insert_path_point(DrawingProgramObjectStore *store,
    ...
    uint32_t move_count;
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-point insert request");
    }
    result = drawing_program_object_store_get_path_payload(store, object_id, &payload);
    if (result.code != CORE_OK) {
        return result;
    ...
    if (payload.point_count >= DRAWING_PROGRAM_OBJECT_PATH_MAX_POINTS) {
        return drawing_program_object_store_invalid("path-point insert exceeds max points");
    }
    if ((uint32_t)insert_index > payload.point_count) {
        return drawing_program_object_store_invalid("path-point insert index out of range");
    }
...
CoreResult drawing_program_object_store_insert_path_point_data(DrawingProgramObjectStore *store,
    ...
    uint32_t move_count;
    if (!store || object_id == 0u || !point) {
        return drawing_program_object_store_invalid("invalid object path-point data insert request");
    }
    result = drawing_program_object_store_get_path_payload(store, object_id, &payload);
    if (result.code != CORE_OK) {
        return result;
    ...
    if (payload.point_count >= DRAWING_PROGRAM_OBJECT_PATH_MAX_POINTS) {
        return drawing_program_object_store_invalid("path-point data insert exceeds max points");
    }
    if ((uint32_t)insert_index > payload.point_count) {
        return drawing_program_object_store_invalid("path-point data insert index out of range");
    }
    move_count = payload.point_count - (uint32_t)insert_index;
    if (move_count > 0u) {
        memmove(&payload.points[insert_index + 1u],
...
CoreResult drawing_program_object_store_remove_path_point(DrawingProgramObjectStore *store,
    ...
    uint32_t move_count;
    if (!store || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-point remove request");
    }
    result = drawing_program_object_store_get_path_payload(store, object_id, &payload);
    if (result.code != CORE_OK) {
        return result;
    ...
    if ((uint32_t)point_index >= payload.point_count) {
        return drawing_program_object_store_invalid("path-point remove index out of range");
    }
    if (out_removed_x) {
    ...
    if (payload.point_count <= 2u) {
        return drawing_program_object_store_invalid("path-point remove requires at least 3 points before removal");
    }
...
CoreResult drawing_program_object_store_get_path_payload(const DrawingProgramObjectStore *store,
    ...
    if (!store || !out_payload || object_id == 0u) {
        return drawing_program_object_store_invalid("invalid object path-payload get request");
    }
    if (drawing_program_object_store_find_index_for_id(store, object_id, &index).code != CORE_OK) {
        return (CoreResult){ CORE_ERR_NOT_FOUND, "object id not found for path payload" };
    }
    if (store->objects[index].type != (uint8_t)DRAWING_PROGRAM_OBJECT_TYPE_PATH) {
        return drawing_program_object_store_invalid("path payload get requires PATH object type");
    }
...
CoreResult drawing_program_object_store_clamp_selection_delta(const DrawingProgramObjectStore *store,
    ...
    if (!store || !document || !selection || !out_dx || !out_dy) {
        return drawing_program_object_store_invalid("invalid object selection clamp request");
    }
...
CoreResult drawing_program_object_store_apply_selection_delta(DrawingProgramObjectStore *store,
    ...
    uint32_t i;
    if (!store || !selection) {
        return drawing_program_object_store_invalid("invalid object selection delta apply request");
    }
    for (i = 0u; i < selection->count && i < DRAWING_PROGRAM_MAX_OBJECTS; ++i) {
...
CoreResult drawing_program_object_store_promote_selection(DrawingProgramObjectStore *store,
    ...
    uint32_t i;
    if (!store || !selection) {
        return drawing_program_object_store_invalid("invalid object selection promote request");
    }
    ...
    for (i = 0u; i < store->object_count && i < DRAWING_PROGRAM_MAX_OBJECTS; ++i) {
        ...
        if (drawing_program_object_store_selection_contains_object_id(selection, object->object_id)) {
            continue;
        }
    ...
    if (write_index != store->object_count) {
        return drawing_program_object_store_invalid("object selection promote reorder mismatch");
    }
...
CoreResult drawing_program_object_store_hit_test_topmost(const DrawingProgramObjectStore *store,
    ...
    if (!store || !out_object_id) {
        return drawing_program_object_store_invalid("invalid object hit-test request");
    }
