...
SDL_Rect right_color_active_swatch_rect(SDL_Rect rect, VisualPaneLayoutMetrics m) {
    int y = right_color_content_start_y(rect, m) + m.line_h + m.section_gap;
    return (SDL_Rect){ rect.x + m.pad_x, y, rect.w - (2 * m.pad_x), m.row_h + m.line_h };
}
...
SDL_Rect right_color_recent_swatch_rect(SDL_Rect rect, VisualPaneLayoutMetrics m, uint8_t recent_index) {
    ...
    int swatch_y = active.y + active.h + m.line_h + (2 * m.section_gap);
